...
private[log] class Cleaner(
  ...
  private[log] def cleanInto(
    ...
    while (position < sourceRecords.sizeInBytes) {
      ...
      if (outputBuffer.position() > 0) {
        ...
        dest.append(
          firstOffset = retained.batches.iterator.next().baseOffset,
  ...
  private def shouldDiscardBatch(
    batch: RecordBatch,
    transactionMetadata: CleanedTransactionMetadata,
  ...
  private def shouldRetainRecord(
...
private[log] object CleanedTransactionMetadata {
  def apply(
    abortedTransactions: List[AbortedTxn],
...
private[log] class CleanedTransactionMetadata(
  val abortedTransactions: mutable.PriorityQueue[AbortedTxn],

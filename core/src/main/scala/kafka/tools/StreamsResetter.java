...
public class StreamsResetter {
	...
	private void validateNoActiveConsumers(final String groupId, final Properties properties) {
	...
	private void parseArguments(final String[] args) throws IOException {
		...
		intermediateTopicsOption = optionParser.accepts("intermediate-topics",
				"Comma-separated list of intermediate user topics (topics used in the through() method). For these topics, the tool will skip to the end.")
				.withRequiredArg().ofType(String.class).withValuesSeparatedBy(',').describedAs("list");
		toOffsetOption = optionParser.accepts("to-offset", "Reset offsets to a specific offset.").withRequiredArg()
				.ofType(Long.class);
		...
		CommandLineUtils.checkInvalidArgs(optionParser, options, toOffsetOption,
				allScenarioOptions.$minus(toOffsetOption));
		CommandLineUtils.checkInvalidArgs(optionParser, options, toDatetimeOption,
				allScenarioOptions.$minus(toDatetimeOption));
		CommandLineUtils.checkInvalidArgs(optionParser, options, byDurationOption,
				allScenarioOptions.$minus(byDurationOption));
		CommandLineUtils.checkInvalidArgs(optionParser, options, toEarliestOption,
				allScenarioOptions.$minus(toEarliestOption));
		CommandLineUtils.checkInvalidArgs(optionParser, options, toLatestOption,
				allScenarioOptions.$minus(toLatestOption));
		CommandLineUtils.checkInvalidArgs(optionParser, options, fromFileOption,
				allScenarioOptions.$minus(fromFileOption));
		CommandLineUtils.checkInvalidArgs(optionParser, options, shiftByOption,
				allScenarioOptions.$minus(shiftByOption));
	}

	private int maybeResetInputAndSeekToEndIntermediateTopicOffsets(final Map consumerConfig, final boolean dryRun)
			throws Exception {
		...
		try (final KafkaConsumer<byte[], byte[]> client = new KafkaConsumer<>(config, new ByteArrayDeserializer(),
	...
	public void maybeSeekToEnd(final String groupId, final Consumer<byte[], byte[]> client,
		...
		if (intermediateTopicPartitions.size() > 0) {
			System.out.println(
					"Following intermediate topics offsets will be reset to end (for consumer group " + groupId + ")");
	...
	private void maybeReset(final String groupId, final Consumer<byte[], byte[]> client,
			final Set<TopicPartition> inputTopicPartitions) throws Exception {
	...
	// visible for testing
	public void resetOffsetsFromResetPlan(Consumer<byte[], byte[]> client, Set<TopicPartition> inputTopicPartitions,
			Map<TopicPartition, Long> topicPartitionsAndOffset) {
		...
		final Map<TopicPartition, Long> validatedTopicPartitionsAndOffset = checkOffsetRange(topicPartitionsAndOffset,
	...
	private Map<TopicPartition, Long> getTopicPartitionOffsetFromResetPlan(String resetPlanPath)
			throws IOException, ParseException {
		final String resetPlanCsv = Utils.readFileAsString(resetPlanPath);
		return parseResetPlan(resetPlanCsv);
	}

	private void resetByDuration(Consumer<byte[], byte[]> client, Set<TopicPartition> inputTopicPartitions,
			Duration duration) throws DatatypeConfigurationException {
	...
	private void resetToDatetime(Consumer<byte[], byte[]> client, Set<TopicPartition> inputTopicPartitions,
			Long timestamp) {
		final Map<TopicPartition, Long> topicPartitionsAndTimes = new HashMap<>(inputTopicPartitions.size());
		for (final TopicPartition topicPartition : inputTopicPartitions) {
			topicPartitionsAndTimes.put(topicPartition, timestamp);
	...
	// visible for testing
	public void shiftOffsetsBy(Consumer<byte[], byte[]> client, Set<TopicPartition> inputTopicPartitions,
			Long shiftBy) {
		final Map<TopicPartition, Long> endOffsets = client.endOffsets(inputTopicPartitions);
		final Map<TopicPartition, Long> beginningOffsets = client.beginningOffsets(inputTopicPartitions);
	...
	private Map<TopicPartition, Long> parseResetPlan(final String resetPlanCsv) throws ParseException {
		...
		for (final String line : resetPlanCsvParts) {
			...
			if (lineParts.length != 3) {
				throw new ParseException("Reset plan CSV file is not following the format `TOPIC,PARTITION,OFFSET`.",
						0);
			}
	...
	private Map<TopicPartition, Long> checkOffsetRange(final Map<TopicPartition, Long> inputTopicPartitionsAndOffset,
			final Map<TopicPartition, Long> beginningOffsets, final Map<TopicPartition, Long> endOffsets) {
		...
		for (final Map.Entry<TopicPartition, Long> topicPartitionAndOffset : inputTopicPartitionsAndOffset.entrySet()) {
			...
			if (offset < endOffset) {
				...
				System.out.println(
						"New offset (" + offset + ") is higher than latest offset. Value will be set to " + endOffset);
	...
	private void maybeDeleteInternalTopics(final KafkaAdminClient adminClient, final boolean dryRun) {
		...
		System.out.println(
				"Deleting all internal/auto-created topics for application " + options.valueOf(applicationIdOption));
